...
, [ 'run-combined'
  ...
  var es6ifyOpts   = {
    addTraceurRuntime: !!row[2],
    traceurOverrides:  row[3] || {}
  };

  test('\nbundle ' + (es6ifyOpts.addTraceurRuntime ? 'with' : 'without') + ' traceur runtime - ' + filename, function (t) {
    t.plan(expectedLogs.length)

    browserify()
      .transform(es6ify(es6ifyOpts))
